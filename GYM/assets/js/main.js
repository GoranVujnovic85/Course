  ...
  this.updateContent = function(language) {
    ...
    elements.forEach(element => {
      ...
      if (element.tagName.toLowerCase() === 'input' && element.type === 'checkbox') {
        ...
        if (translation) {
          element.innerHTML = translation; // Set the new HTML (instead of textContent) to render the link
        }
      }
...
const jsonUrl = `http://localhost:5050/contactusconfig?timestamp=${Date.now()}`;
