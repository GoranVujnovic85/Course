...
app.get('/maintenanceconfig', async (req, res) => {
    try {
...
app.get('/contactusconfig', async (req, res) => {
    try {
        // Authenticate the database connection first
        await sequelize.authenticate();
        console.log('Connection has been established successfully.');

        // Define the ContactUs model here, after the connection is successful
        ...
        // Sync the ContactUs table
        await ContactUs.sync();

        // Sample data to insert (if needed)
        const locations = [
            {
        ...
        // Optionally, insert locations into the database (for persistence)
        for (let location of locations) {
            await ContactUs.create(location);
        }
...
// Define the Locker model based on your data
const Locker = sequelize.define('Locker', {
    ...
    // Sync the Locker table (create if it doesn't exist)
    await Locker.sync();
    ...
    // Insert sample data into the Locker table (if needed)
    for (let locker of lockerData) {
    await Locker.create(locker);
}

// Fetch all locker data
const lockers = await Locker.findAll();

// Send the locker data in the response
res.json({ results: lockers });
...
app.get('/images', async (req, res) => {
    try {
        ...
        // Sync the Images table (create if it doesn't exist)
        await Images.sync();
    ...
    const sampleImages = [
        {
            ...
            active: true,
            createdAt: new Date("Mon Nov 25 2024 11:26:46 GMT+0100 (Central European Standard Time)")
        },
        {
   ...
   // Bulk create records if they don't exist
   await Images.bulkCreate(sampleImages);

   // Fetch data from the database
   const images = await Images.findAll();

   // Convert Sequelize instances to plain JavaScript objects
   const imagesData = images.map(image => image.toJSON());

   // Return the data in the expected format
   res.json({ results: imagesData });
...
app.get('/worktimedata', async (req, res) => {
    try {
        // Authenticate the database connection first
        await sequelize.authenticate();
        console.log('Connection has been established successfully.');

        // Define the WorkTime model
        ...
        // Sync the WorkTime and Notice tables (create if they don't exist)
        await WorkTime.sync();
        await Notice.sync();

        // Sample WorkTime data
        ...
        await WorkTime.bulkCreate(sampleWorkTime);
        await Notice.create(sampleNotice);
        ...
        // Convert Sequelize instances to plain JavaScript objects
        const workTimeData = workTimes.map(workTime => workTime.toJSON());
        const noticeData = notices.map(notice => notice.toJSON());
